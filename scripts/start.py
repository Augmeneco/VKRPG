class Start:
    def __init__(self):
        ...
        vkrpg.events.add_event('on_message', self.main, 'default')
    ...
    def main(self, msg):
        ...
        if msg['pure_text'].lower() == 'старт':
            ...
            username = self.req.post('https://api.vk.com/method/users.get',
                                     data={'access_token': TOKEN, 'v': '5.84', 'user_ids': msg['from_id']}).json()
            ...
            servtext = self.json.loads(open('data/text.json', 'r').read())
            userserv = self.random.choice(list(dict(servtext).keys()))
            userservdb = userserv
...
vkrpg.plugins.register_plugin('start', Start())